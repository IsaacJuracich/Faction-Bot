...
namespace orbitFactionBot.Protocol
{
    /// <summary>
    ...
    public class Replay
    {
        ...
        public Replay(int protocolVersion)
        {
            Initialize(protocolVersion);
        }

        public Replay(int protocolVersion, string serverName, string recordingDirectory = @"replay_recordings")
        {
            Initialize(protocolVersion);
        ...
        {
            string name = orbitFactionBot.Utils.Converter.getIGN(uuid.ToString());
            if (!string.IsNullOrEmpty(name)) {
            }
            else MetaData.AddPlayerUUID(uuid);
        }
        ...
        private void HandleInBoundPacket(int packetID, IEnumerable<byte> packetData, bool isLogin)
        {
            ...
            if (pType == PacketTypesIn.TabComplete) {
            }
